...
function MapEffects({ center, radiusKm, results, selected, searchToken }: MapEffectsProps) {
  const map = useMap();
...
export default function MapView({ center, radiusKm, results, selected, searchToken }: MapViewProps) {
  const initialCenter: [number, number] = center ? [center.lat, center.lon] : HOUSTON;

  return (
    <MapContainer center={initialCenter} zoom={DEFAULT_ZOOM} className="leaflet-container" scrollWheelZoom>
      <TileLayer
      ...
      <MapEffects center={center} radiusKm={radiusKm} results={results} selected={selected} searchToken={searchToken} />
    </MapContainer>
  );
}
